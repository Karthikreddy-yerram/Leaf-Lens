"""
MongoDB Setup for Leaf-Lens Application
This module handles the connection to MongoDB and provides database access functions.
"""
...
def hash_password(password):
    ...
    if BCRYPT_AVAILABLE:
        ...
        # Simple (insecure) password "hashing" if bcrypt not available
        import hashlib
...
def verify_password(stored_password, provided_password):
    ...
    if BCRYPT_AVAILABLE:
        if isinstance(stored_password, str):
            # Handle case where password was saved as string and not bytes
            return provided_password == stored_password
        return bcrypt.checkpw(provided_password.encode('utf-8'), stored_password)
    else:
        # Simple verification for non-bcrypt passwords
        import hashlib
...
def setup_database():
    ...
    try:
        ...
        # Add sample plants if the collection is empty
...
def save_plant_history(user_id, plant_data):
    """Save a plant identification to user history."""
    if not MONGO_CONNECTED:
        # Fallback to in-memory storage
        history_id = str(uuid.uuid4())
        history_entry = {
            "_id": history_id,
            "id": history_id,  # Also include id field for compatibility
            "userId": user_id,
    ...
    try:
        # Ensure plant data has the correct fields
        history_entry = {
            "userId": user_id,
            "plantName": plant_data.get("plantName"),
            "confidence": plant_data.get("confidence"),
...
def get_user_history(user_id):
    ...
    try:
        ...
        for item in user_history:
            item["_id"] = str(item["_id"])
            # Ensure each item has an id field that's a string
